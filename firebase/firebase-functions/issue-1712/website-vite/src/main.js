import './style.css'
import { triggerEvent } from './firebase.js'
...
document.querySelector('#app').innerHTML = `
  <div class="container">
    ...
    <div class="status">
      ...
      <ul>
        <li>This page will automatically trigger the analytics event when loaded</li>
...
function handleTriggerEvent() {
  log('🔄 Triggering analytics event...');
  const success = triggerEvent();
  
  if (success) {
    updateStatus('Analytics event triggered! Check Firebase Functions logs.', 'success');
...
// Automatically trigger analytics event after a short delay
setTimeout(() => {
  log('🔄 Auto-triggering analytics event...');
  handleTriggerEvent();
}, 1000);
